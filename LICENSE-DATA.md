...
- The org definition files in `src/tessera/examples/` (`toy_org.py` and
  `meridian_org.py`) are **dual-licensed**: as Python source they
...
- Blueprint JSON exported from those orgs (e.g. through the API's blueprint store),
  including the bundled `src/tessera/data/templates/suite.json` starter,
