"""Credential-safe error scrubbing regressions."""
...
def test_the_self_grading_guard_message_keeps_its_existing_bare_shape():
...
def test_a_credential_from_a_provider_tessera_does_not_know_is_still_redacted(monkeypatch):
    # RunSpec.model is free text by design, so a run can target any inspect_ai
...
def test_scrubbing_twice_changes_nothing(monkeypatch):
    # Callers may defensively scrub at multiple boundaries; the double pass must be inert.
    from tessera.api.scrub import scrub_error
