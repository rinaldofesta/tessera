...
import { DATASET_DESCRIPTIONS, WIZARD_COPY } from "@/copy";
      ...
      >
        {suites.map((suite) => {
          ...
          return (
            ...
            >
              <h3 className="font-display text-[15px] font-bold">{suite.id}</h3>
              {DATASET_DESCRIPTIONS[suite.id] && (
                <p className="mt-1.5 text-[12.5px] leading-relaxed text-[var(--muted-foreground)]">
                  {DATASET_DESCRIPTIONS[suite.id]}
                </p>
              )}
              <p className="mt-2.5 font-mono text-[10.5px] text-[var(--faint)]">
